...
ENV ASF_PATH=/app
ENV ASF_USER=asf
# Corrigido para garantir que o ASF escute na porta 1242
...
WORKDIR /app
# Adicionando COPY para a pasta config ANTES do VOLUME, para que a configuração inicial possa ser copiada
# Se você estiver usando um volume persistente no Render, este COPY será sobrescrito pelo volume,
# mas é útil para ter uma config inicial na imagem se não houver volume ou para testes.
